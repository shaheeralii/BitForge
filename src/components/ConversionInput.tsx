...
export const ConversionInput: React.FC<ConversionInputProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-[var(--bf-surface)] rounded-xl border border-slate-200 dark:border-[var(--bf-muted)]/40 p-5 sm:p-6 shadow-sm transition-all space-y-5">
      
      {/* Top Bar: Source Base Selector & Auto Detect Status */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <label htmlFor="bitforge-main-input" className="text-xs font-bold uppercase tracking-wider text-[var(--bf-muted)] dark:text-[var(--bf-accent)]">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-md border transition-colors ${
              isLocked
                ...
                : 'bg-[#F4FAF9] dark:bg-[var(--bf-chip-alt)] text-[var(--bf-muted)] dark:text-[var(--bf-heading)] border-slate-200 dark:border-[var(--bf-muted)]/50 hover:bg-slate-100 dark:hover:bg-[var(--bf-chip-alt-hover)]'
            }`}
        ...
        {!isLocked && inputVal.trim() && (
          <div className="flex items-center gap-2 text-xs bg-[var(--bf-heading)] dark:bg-[var(--bf-chip)] border border-[var(--bf-accent)]/40 px-3 py-1 rounded-md text-[var(--bf-chip)] dark:text-[var(--bf-heading)] font-mono">
            <Sparkles className="w-3.5 h-3.5 text-[var(--bf-accent-dark)] dark:text-[var(--bf-accent)] animate-pulse shrink-0" />
            <span className="font-sans font-semibold text-[var(--bf-muted)] dark:text-[var(--bf-accent)]">AUTO-DETECTED:</span>
            <span className="font-bold uppercase text-[var(--bf-chip)] dark:text-white">
              {BASE_OPTIONS[autoDetect.detectedBase]?.name || `Base ${autoDetect.detectedBase}`}
            </span>
            <span className="text-[var(--bf-muted)] dark:text-slate-300 text-[11px] hidden md:inline font-sans">
      ...
      <div className="relative group">
        <input
          ...
          className={`w-full font-mono text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight px-4 sm:px-6 py-4 rounded-xl border-2 transition-all outline-none bg-slate-50 dark:bg-[var(--bf-surface-deep)] ${
            errorMessage
              ? 'border-rose-300 dark:border-rose-800 text-rose-900 dark:text-rose-200 focus:ring-2 focus:ring-rose-500/20'
              : 'border-slate-200 dark:border-[var(--bf-muted)]/60 text-[var(--bf-chip)] dark:text-[var(--bf-heading)] focus:border-[var(--bf-accent)] dark:focus:border-[var(--bf-accent)] focus:bg-white dark:focus:bg-[var(--bf-overlay)] focus:ring-2 focus:ring-[var(--bf-accent)]/20'
          }`}
        />
        ...
        <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
          {inputVal && (
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-[var(--bf-chip)] rounded-lg transition-colors"
              title="Clear Input"
          ...
          <span className="hidden sm:inline-block font-mono font-bold text-xs text-[var(--bf-chip)] dark:text-[var(--bf-accent)] bg-[var(--bf-heading)] dark:bg-[var(--bf-chip)] px-2.5 py-1 rounded border border-[var(--bf-accent)]/40 uppercase">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between w-full mb-1">
                <span className={`text-[10px] font-bold uppercase tracking-wider ${isSelected ? 'text-[var(--bf-heading)]' : 'text-[var(--bf-muted)] dark:text-slate-400'}`}>
              ...
              <span className={`text-xs font-bold truncate w-full ${isSelected ? 'text-white' : 'text-[var(--bf-chip)] dark:text-slate-100'}`}>
      ...
      {sourceBase === 'custom' && (
        <div className="flex items-center gap-4 bg-[#F4FAF9] dark:bg-[var(--bf-chip-alt)] p-3 rounded-lg border border-slate-200 dark:border-[var(--bf-muted)]/40">
          <label className="text-xs font-bold text-[var(--bf-chip)] dark:text-[var(--bf-heading)] whitespace-nowrap">
          ...
          <input
            ...
            className="w-16 px-2 py-1 text-center font-mono font-bold text-sm bg-white dark:bg-[var(--bf-surface-deep)] border border-slate-300 dark:border-[var(--bf-muted)] rounded-md text-[var(--bf-chip)] dark:text-[var(--bf-accent)]"
          />
        </div>
      )}
