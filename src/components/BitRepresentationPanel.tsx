...
const Note: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="flex items-start gap-2 text-[11px] text-[var(--bf-heading)]/65 border-l-2 border-[var(--bf-muted)]/50 pl-2.5 py-0.5">
  ...
  <div className="space-y-2.5">
    {steps.map((step, idx) => (
      <div key={idx} className="rounded-lg border border-[var(--bf-muted)]/40 bg-[var(--bf-surface-inset)] p-3 space-y-1.5">
        ...
        <p className="text-xs text-[var(--bf-heading)]/80 leading-relaxed">{step.explanation}</p>
        {step.equationLines && (
        ...
        {step.finalResult && (
          <div className="flex items-center justify-between text-xs font-mono font-bold bg-[var(--bf-chip-alt)] px-2.5 py-1.5 rounded-md">
            <span className="text-[var(--bf-heading)]/65 font-sans font-normal">Result:</span>
  ...
  if (representation === 'unsigned') {
    return (
      <div className="space-y-4">
        <p className="text-sm text-[var(--bf-heading)]/80 leading-relaxed">
