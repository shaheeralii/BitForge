...
const CopyMessageButton: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <button
      onClick={handleCopy}
      className="flex items-center gap-1 text-[10px] text-[#D9FFF4]/40 hover:text-[#34E89A] transition-colors mt-1"
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={onOpen}
          className="fixed bottom-5 right-5 z-40 flex items-center gap-2 pl-3.5 pr-4 py-3 rounded-full bg-[#34E89A] text-[#0A3324] shadow-lg shadow-[#34E89A]/30 hover:shadow-xl hover:shadow-[#34E89A]/40 hover:scale-105 transition-all font-bold text-sm"
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3.5 border-b border-[#1F6B4C]/30 shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                <div className="p-1.5 rounded-lg bg-[#0A3324] text-[#34E89A] shrink-0">
                  <Bot className="w-4 h-4" />
                </div>
                <div className="min-w-0">
                  <h2 className="text-sm font-display font-semibold tracking-wide text-[#D9FFF4] leading-tight truncate">BitForge AI</h2>
                  <p className="text-[10px] text-[#D9FFF4]/60 truncate">Your CS learning assistant</p>
                </div>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
              {messages.length === 0 && (
                <div className="text-center py-6">
                  <Bot className="w-8 h-8 text-[#34E89A]/50 mx-auto mb-2" />
                  <p className="text-xs text-[#D9FFF4]/60 leading-relaxed px-2">
                ...
                <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`max-w-[85%] rounded-xl px-3 py-2 text-[13px] leading-relaxed ${
                      m.role === 'user'
                        ? 'bg-[#34E89A] text-[#0A3324] font-medium'
                        : m.isError
                        ? 'bg-red-950/40 border border-red-500/30 text-red-200'
                        : 'bg-black/25 border border-[#1F6B4C]/30 text-[#D9FFF4]/90'
                    }`}
                  >
              ...
              {isSending && (
                <div className="flex justify-start">
                  <div className="bg-black/25 border border-[#1F6B4C]/30 rounded-xl px-3 py-2 flex items-center gap-2">
                    <Loader2 className="w-3.5 h-3.5 text-[#34E89A] animate-spin" />
                    <span className="text-[11px] text-[#D9FFF4]/60">Thinking…</span>
                  </div>
                </div>
              )}
            </div>
                ...
                {QUICK_PROMPTS.map((qp) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-black/20 hover:bg-[#0A3324] text-[#D9FFF4]/70 hover:text-[#34E89A] border border-[#1F6B4C]/40 hover:border-[#34E89A]/40 text-[11px] font-medium transition-colors"
                    >
            ...
            {/* Input */}
            <div className="p-3 border-t border-[#1F6B4C]/30 shrink-0">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="flex-1 resize-none bg-black/25 border border-[#1F6B4C]/40 focus:border-[#34E89A]/60 rounded-lg px-3 py-2 text-[13px] text-[#D9FFF4] placeholder:text-[#D9FFF4]/30 outline-none transition-colors max-h-24"
                />
                <button
                  onClick={() => submit(draft)}
                  disabled={!draft.trim() || isSending}
                  className="p-2.5 rounded-lg bg-[#34E89A] text-[#0A3324] disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#2fd08a] transition-colors shrink-0"
              ...
              <div className="flex items-center justify-between mt-1.5 px-0.5">
                <p className="text-[9px] text-[#D9FFF4]/35">AI-generated — verify anything critical.</p>
                {nearLimit && (
                  <p className={`text-[9px] font-mono ${draft.length >= maxMessageLength ? 'text-red-400' : 'text-[#D9FFF4]/40'}`}>
